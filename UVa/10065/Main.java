...
public class Main
{
    public static void main(String[] args) throws IOException
    {
        ...
        while(true)
        {
            ...
            pts[0] = pts[n] = new Point(sc.nextInt(), sc.nextInt());
            for(int i = 1; i < n; i++)
                pts[i] = new Point(sc.nextInt(), sc.nextInt());

            double tile = new Polygon(pts).area();
            double total = Polygon.convexHull(Arrays.copyOf(pts, n)).area();
            double ans = 100.0 - ((tile / total) * 100.0);
            pw.println("Tile #" + (t++));
            pw.printf("Wasted Space = %.2f", ans);
            pw.println(" %");
            pw.println();
        }
